    ...
    $.createLayoutManager = function(p) {
        ...
        var LayoutManager = {
            ...
            init: function() {
                ...
                SageFrame.tooltip.GetToolTip("imgToolTip1", "#spnActivelayout", "This is from where you can include Items into you menu");
                SageFrame.tooltip.GetToolTip("imgToolTip2", "#spnActivetheme", "This is from where you can include Items into you menu");
                SageFrame.tooltip.GetToolTip("imgToolTip3", "#spnActivewidth", "This is from where you can include Items into you menu");
                SageFrame.tooltip.GetToolTip("imgToolTip4", "#spnHandheld", "This is from where you can include Items into you menu");
                SageFrame.tooltip.GetToolTip("imgToolTip5", "#spnApplytopages", "This is from where you can include Items into you menu");
                ...
                $('#lblSaveLayoutChange').live("click", function() {
                    LayoutManager.config.PanName = [];
                    LayoutManager.config.NewValue = [];
                    $('.sfOuterwrapper').each(function(index) {
                        if ($(this).attr('id') !== 'sfBodyContent') {
                            var tableId = $(this).attr('id') + "_mytable";
                            LayoutManager.LoadPanName($(this).attr('id'));
                    ...
                    $('#sfOuterWrapper div.sfOuterwrapper').each(function(index) {
                        ...
                        if (id !== "sfBodyContent") {
                            ...
                            $('.sfMainContent,.sfMiddletop,.sfMiddlebottom,.sfFulltopspan,.sfFullbottomspan').find('table').each(function() {
                                var pid = "#divPlaceHolder_" + $(this).attr('id');
                                containA += $(pid).text();
                            });
                        }
                        if (temp == 0) {
                ...
                $('#txtName,#txtWidth,#txtWrapinner,#txtWrapouter').live("keyup", function() {
                    var self = $(this);
                    $.each(LayoutManager.config.pchArr, function(index, item) {
                        var controlKey = $(self).attr("title");
                        if (index > 0) {
                ...
                $('#txtPlaceholder').live("keyup", function() {
                ...
                $('#txtWrapName,#txtWrapClass,#txtWrapDepth').live("keyup", function() {
                    var self = $(this);
                ...
                $('#selWraptype').live("change", function() {
                    var self = $(this);
                    $.each(LayoutManager.config.wrapArr, function(index, item) {
                        var controlKey = $(self).attr("title");
                ...
                $('#txtPositions').live("keyup", function() {
                ...
                $('div.sfPresetpages span.delete').live("click", function() {
                    $(this).parent("li").remove();
                });
                $('#activeLayoutList select').live("change", function() {
                    $(this).parent().find("img.sfAddpage").remove();
                    if ($(this).val() == "2") {
                        $(this).parent().append('<span class="customize"><img src=' + SageFrame.utils.GetAdminImage("page-edit.png") + ' class="sfAddpage" alt="Edit Width" /></span>');
            ...
            BindEvents: function() {
                ...
                $('li.liDelete').live('click', function() {
                    var selDiv = $(this).attr("id").replace("del_", "");
                ...
                $('#spApply').live('click', function() {
                    var FileName = $('#ddlLayoutList option:selected').val();
                    var FilePath = FileName;
                    setXml(FilePath);
                });
                $('#imgUndo').live('click', function() {
                    var restore = $('#ddlDeletelist option:selected').val();
                    $('#del_' + restore).parents('div .remove').attr('class', restore);
                ...
                $('#imgAddPlaceHolder').toggle(function() {
                    ...
                    $('#imgAddPlaceHolder').attr("src", "/Modules/LayoutManager/images/allow_list_close.jpg");
                }, function() {
                    $('#txtLayoutEditor').removeClass("layoutEditorWithPch").addClass("layoutEditor");
                    $('#tblPch').hide();
                    $('#imgAddPlaceHolder').attr("src", "/Modules/LayoutManager/images/text_code_add.png");
                });
                $('#pchAdd').live('click', function() {
                    $(editor).insertAtPch($('#pchPreview').html());
                });

                // themes
                $("#layoutList table tr img.edit").live("click", function() {
                    var fileName = $(this).prev("input[type='hidden']").val();
                    LayoutManager.ReadLayout(fileName);
                });
                $("#sectionList table tr img.edit").live("click", function() {
                    var fileName = $(this).prev("input[type='hidden']").val();
                ...
                $('#pageTree li span').live("click", function() {
                ...
                $('#spnSavePagePreset').bind("click", function() {
                    ...
                    $.each(pages, function(index, item) {
                        html += '<li class="sfCurve">' + $.trim($(this).text().replace(/-/g, " ")).replace(' ', '-') + '<span class="sfDelete"><img src=' + SageFrame.utils.GetAdminImage("imgdelete.png") + ' /></span></li>';
                    });
                    html += '</ul></div>';
                ...
                $('span.customize').live("click", function() {
                    LayoutManager.config.ActivePageDiv = $(this).prev("select");
                    LayoutManager.ShowPopUp("divPagePresets", "Select Pages");
                    LayoutManager.GetPages();
                });
                $('#presetPages li span.delete').live("click", function() {
                    $(this).parent("li").remove();
                });
                $('div.sfButtonwrapper li.activate').live("click", function() {
                    var template = $(this).find("a").attr("id");
                    template = template.substring(template.indexOf('#') + 1, template.length);
                    LayoutManager.ActivateTemplate(template);
                });
                $('div.sfButtonwrapper li.preview').live("click", function() {
                    var template = $(this).find("a").attr("id");
                    template = template.substring(template.indexOf('#') + 1, template.length);
                ...
                $('a.sfTemplatethumb').live("click", function() {
                ...
                $('div.sfTemplatedetail').live("click", function() {
                ...
                $('#ddlLayoutList').live("change", function() {
                ...
                $('#lblSaveLayout').live("click", function() {
                    var fileName = $('#ddlLayoutList').val();
                ...
                $('#divFileList ul li a').live("click", function() {
                ...
                $('#divFileMgrBreadCrumb span').live("click", function() {
                ...
                $('#btnCreatePreset').live("click", function() {
                    var html = "";
                ...
                $('div.sfPresetpages span.sfDelete').live("click", function() {
            ...
            HidePopUp: function() {
                $('#divEditXML,#divAddLayout,#divPagePresets').dialog("close");
            },
            ...
            BindThemes: function(data) {
                ...
                $('#themeList img.sfDelete').click(function() {
                    var dName = $(this).attr("id").replace("del_", "");
            ...
            GetPages: function() {
                ...
                this.config.data = JSON2.stringify({ PortalID: p.PortalID, UserName: 'superuser', CultureCode: 'en-US' });
            ...
            BindTemplates: function(data) {
                ...
                $.each(template, function(index, item) {
                    ...
                    tempMarkup += '<div class="sfButtonwrapper"><ul><li class="activate"><a href="#" id=' + activateId + '>Activate</a></li>';
                    var editFileLink = p.EditFilePath + '/Admin/Template-File-Editor' + p.Extension + '?tname=' + item.TemplateSeoName;
                    if (!item.IsDefault) {
                        tempMarkup += '<li class="sfEditfiles"><a href=' + editFileLink + ' id="lnkEditFiles">EditFiles</a></li>';
                        if (!item.IsActive)
                ...
                $('div.sfButtonwrapper li.sfDelete').click(function() {
                    var template = $(this).find("a").attr("rel");
            ...
            LoadLayoutWireFrame: function(layout) {
                ...
                this.config.url = this.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ FilePath: layout, TemplateName: LayoutManager.config.Template });
            ...
            InitResizeLayout: function() {
                ...
                $('.sfOuterwrapper').each(function(index) {
                    if ($(this).attr('id') !== 'sfBodyContent') {
                        var tableId = $(this).attr('id').toLowerCase() + "_mytable";
            ...
            SortPan: function() {
                $('#sfOuterWrapper').sortable({
                    //revert: true
                    items: ":not(#sfBodyContent,#sfOuterWrapper div div,#sfOuterWrapper div table thead tr th div,#sfOuterWrapper div table thead tr th,#sfOuterWrapper div table thead tr,#sfOuterWrapper div table thead,#sfOuterWrapper div table)",
                    update: function() {
                    }
                });
                $('table thead tr').sortable({ connectWith: '#' + $(this).parents().find('table').attr('id'),
                    stop: function() {
                        $("#" + $(this).parents().find('table').attr('id')).resize();
                    }
                });
                    ...
                    $.each(activelayouts, function(index, item) {
                        if ((self).text().toLowerCase() == item.Key.toLowerCase()) {
                            ...
                            if (item.Value.toLowerCase() == "all") {
                                ...
                                $(self).parent().append('<span class="customize"><img src=' + SageFrame.utils.GetAdminImage("page-edit.png") + ' class="sfAddpage" alt="Edit Width" /></span>');
                                var pages = item.Value.split(',');
                                var html = '<div class="sfPresetpages"><ul>';
                                $.each(pages, function(i, it) {
                                    html += '<li class="sfCurve">' + SageFrame.utils.GetPageSEOName(it) + '<span class="sfDelete"><img src=' + SageFrame.utils.GetAdminImage("imgdelete.png") + ' /></span></li>';
                                });
                                html += '</ul></div>';
            ...
            ClearPresetDetails: function() {
                ...
                $('#spnActiveTheme').text("");
                $('#spnActiveWidth').text("");
                $('#chkCssOpt_preset').attr("checked", false);
                $('#chkJsOpt_preset').attr("checked", false);
                $('#chkShowCpanel_preset').attr("checked", false);
                $('#chkEnableHandheld_preset').attr("checked", false);
                $('#trHandheld').hide();
            },
            SavePreset: function() {
                var param = {
